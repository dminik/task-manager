...
export function NavBar({ collapsed, onToggleNavbar }: NavBarProps) {
  ...
  const mainLinks = mainLinksData.map((item) => MakeLink(item, active, setActive, collapsed));

  const bottomLinks = bottomLinksData.map((item) => MakeLink(item, active, setActive, collapsed));
  return (
